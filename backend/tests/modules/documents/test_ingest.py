...
async def test_full_runner_sequence_indexes_document(
    ...
    assert all(job.finished_at is not None and job.progress == 1.0 for job in jobs.values())
    ...
    assert (
        await session.scalar(
            ...
            .where(AuditEvent.action == "document.version.approved")
        )
        == 1
    )
...
async def test_late_worker_failure_cannot_regress_approved_legacy_content(
    ...
    await mark_failed(document_id, attempt_revision, "late duplicate task failure")
...
async def test_mark_failed_records_reason(
    ...
    assert (
        await session.scalar(
            select(func.count())
            .select_from(AuditEvent)
            .where(AuditEvent.action == "document.version.failed")
        )
        == 1
    )


async def test_lifecycle_audit_failure_rolls_back_failure_transition(
    session: AsyncSession,
    stack_env: None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _context, _workspace, document = await upload(session, "atomic-ingest-failure")
    version = await session.get(DocumentVersion, document.id)
    assert version is not None
    job = IngestJob(
    ...
    async def fail_audit(*_args, **_kwargs) -> None:  # type: ignore[no-untyped-def]
        raise RuntimeError("audit unavailable")

    monkeypatch.setattr(ingest, "record_audit", fail_audit)
    ...
    stored_job = await session.get(IngestJob, job_id)
    assert stored_document is not None
    assert (stored_document.status, stored_document.error) == ("queued", None)
    assert stored_version is not None
    assert (stored_version.state, stored_version.lifecycle_revision) == (
        "processing",
        1,
    )
...
async def test_deletion_finalization_locks_document_before_version(
    ...
    async with factory() as blocker:
        await blocker.execute(select(Document).where(Document.id == document.id).with_for_update())
        ...
        try:
            ...
            while asyncio.get_running_loop().time() < deadline and not finalizer_is_waiting:
...
async def test_rejected_decision_history_retains_metadata_tombstone_after_purge(
    ...
    await session.commit()
    await service.reject_version(session, context, version.id, reason="Superseded draft content")
    await service.request_document_deletion(session, context, version.id)
    version_id = version.id
...
async def test_delete_restarts_after_external_cleanup_before_sql_finalization(
    ...
    assert "qdrant" in first_calls and any(call.startswith("storage:") for call in first_calls)
...
async def test_delete_request_replay_by_another_admin_preserves_original_authority(
    ...
    version_id = version.id
    original = await service.request_document_deletion(session, context, version_id)
    original_requested_at = original.source_delete_requested_at
    replay = await service.request_document_deletion(session, second_context, version_id)
...
@pytest.mark.parametrize("state", ["processing", "review", "approved", "superseded", "obsolete"])
