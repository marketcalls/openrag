"""Attested run-command ingestion that queues work without executing LLM I/O."""

from collections.abc import Mapping, Sequence
from typing import Literal, Protocol

from sqlalchemy import func, update
from sqlalchemy.ext.asyncio import AsyncSession, async_sessionmaker

from openrag.modules.events.consumer import (
    ConsumerRedis,
    ConsumerResult,
    EventAuthorityError,
    SchemaParser,
    StreamDelivery,
    consume_one,
)
from openrag.modules.events.envelopes import (
    RUN_CANCEL_REQUESTED_EVENT_TYPE,
    RUN_REQUESTED_EVENT_TYPE,
    EventEnvelopeBase,
    RunCancelRequestedV1,
    RunRequestedV1,
    parse_base_envelope,
    parse_registered_envelope,
)
from openrag.modules.events.streams import (
    RUN_COMMANDS_DLQ_STREAM,
    RUN_COMMANDS_GROUP,
    RUN_COMMANDS_STREAM,
)
from openrag.modules.runs.models import AgentRun

RUN_COMMAND_CONSUMER = "run-commands-v1"
_MAX_BATCH = 100
RunCommandAction = Literal["queue", "cancel"]
RunCommandPayload = RunRequestedV1 | RunCancelRequestedV1


class RunCommandRedis(ConsumerRedis, Protocol):
    async def xautoclaim(self, **kwargs: object) -> object: ...

    async def xreadgroup(self, **kwargs: object) -> object: ...

    async def xpending_range(self, **kwargs: object) -> object: ...


def parse_run_command(encoded: bytes) -> EventEnvelopeBase:
    envelope = parse_registered_envelope(encoded)
    if envelope.event_type not in {
        RUN_REQUESTED_EVENT_TYPE,
        RUN_CANCEL_REQUESTED_EVENT_TYPE,
    }:
        raise ValueError("schema_not_registered")
    return parse_base_envelope(encoded)


RUN_COMMAND_SCHEMA_PARSERS: Mapping[tuple[int, str], SchemaParser] = {
    (1, RUN_REQUESTED_EVENT_TYPE): parse_run_command,
    (1, RUN_CANCEL_REQUESTED_EVENT_TYPE): parse_run_command,
}


def command_payload(
    envelope: EventEnvelopeBase,
) -> tuple[RunCommandAction, RunCommandPayload]:
    if envelope.event_type == RUN_REQUESTED_EVENT_TYPE:
        return "queue", RunRequestedV1.model_validate(envelope.payload)
    if envelope.event_type == RUN_CANCEL_REQUESTED_EVENT_TYPE:
        return "cancel", RunCancelRequestedV1.model_validate(envelope.payload)
    raise EventAuthorityError("event_not_authoritative")


async def revalidate_run_command(
    session: AsyncSession,
    envelope: EventEnvelopeBase,
) -> None:
    action, payload = command_payload(envelope)
    run = await session.get(AgentRun, envelope.aggregate_id, with_for_update=True)
    valid = (
        run is not None
        and envelope.aggregate_type == "agent_run"
        and envelope.lifecycle_revision == 1
        and run.id == payload.run_id == envelope.aggregate_id
        and run.org_id == envelope.org_id
        and run.workspace_id == envelope.workspace_id
        and run.user_id == payload.user_id
    )
    if valid and action == "queue":
        if not isinstance(payload, RunRequestedV1):
            raise EventAuthorityError("event_not_authoritative")
        requested = payload
        valid = (
            run is not None
            and run.chat_id == requested.chat_id
            and run.input_message_id == requested.input_message_id
            and run.client_request_id == requested.client_request_id
            and run.model_id == requested.model_id
        )
    if not valid:
        raise EventAuthorityError("event_not_authoritative")


async def apply_run_command(
    session: AsyncSession,
    envelope: EventEnvelopeBase,
) -> None:
    action, payload = command_payload(envelope)
    if action == "queue":
        await session.execute(
            update(AgentRun)
            .where(
                AgentRun.id == payload.run_id,
                AgentRun.status == "accepted",
                AgentRun.cancel_requested_at.is_(None),
            )
            .values(status="queued")
        )
        return
    await session.execute(
        update(AgentRun)
        .where(
            AgentRun.id == payload.run_id,
            AgentRun.status.in_(("accepted", "queued", "running")),
            AgentRun.cancel_requested_at.is_(None),
        )
        .values(cancel_requested_at=func.timezone("UTC", func.now()))
    )


async def consume_run_command(
    session_factory: async_sessionmaker[AsyncSession],
    redis: ConsumerRedis,
    delivery: StreamDelivery,
) -> ConsumerResult:
    return await consume_one(
        session_factory,
        redis,
        consumer=RUN_COMMAND_CONSUMER,
        delivery=delivery,
        revalidate=revalidate_run_command,
        apply_effect=apply_run_command,
        poison_delivery_limit=8,
        schema_parsers=RUN_COMMAND_SCHEMA_PARSERS,
        expected_stream=RUN_COMMANDS_STREAM,
        expected_group=RUN_COMMANDS_GROUP,
        dlq_stream=RUN_COMMANDS_DLQ_STREAM,
    )


def _text(value: object) -> str:
    if isinstance(value, bytes):
        try:
            return value.decode("ascii")
        except UnicodeDecodeError as exc:
            raise RuntimeError("run_command_stream_invalid") from exc
    if isinstance(value, str):
        return value
    raise RuntimeError("run_command_stream_invalid")


def _messages(value: object) -> list[tuple[str, Mapping[bytes, bytes]]]:
    if not isinstance(value, Sequence) or isinstance(value, (str, bytes)):
        raise RuntimeError("run_command_stream_invalid")
    messages: list[tuple[str, Mapping[bytes, bytes]]] = []
    for item in value:
        if (
            not isinstance(item, Sequence)
            or isinstance(item, (str, bytes))
            or len(item) != 2
            or not isinstance(item[1], Mapping)
        ):
            raise RuntimeError("run_command_stream_invalid")
        fields = item[1]
        if not all(
            isinstance(key, bytes) and isinstance(field, bytes)
            for key, field in fields.items()
        ):
            raise RuntimeError("run_command_stream_invalid")
        messages.append((_text(item[0]), fields))
    return messages


def _claimed_messages(value: object) -> list[tuple[str, Mapping[bytes, bytes]]]:
    if (
        not isinstance(value, Sequence)
        or isinstance(value, (str, bytes))
        or len(value) < 2
    ):
        raise RuntimeError("run_command_stream_invalid")
    return _messages(value[1])


def _fresh_messages(value: object) -> list[tuple[str, Mapping[bytes, bytes]]]:
    if not isinstance(value, Sequence) or isinstance(value, (str, bytes)):
        raise RuntimeError("run_command_stream_invalid")
    messages: list[tuple[str, Mapping[bytes, bytes]]] = []
    for stream_result in value:
        if (
            not isinstance(stream_result, Sequence)
            or isinstance(stream_result, (str, bytes))
            or len(stream_result) != 2
            or _text(stream_result[0]) != RUN_COMMANDS_STREAM
        ):
            raise RuntimeError("run_command_stream_invalid")
        messages.extend(_messages(stream_result[1]))
    return messages


def _delivery_counts(value: object) -> dict[str, int]:
    if not isinstance(value, Sequence) or isinstance(value, (str, bytes)):
        raise RuntimeError("run_command_pending_invalid")
    counts: dict[str, int] = {}
    for row in value:
        if not isinstance(row, Mapping):
            raise RuntimeError("run_command_pending_invalid")
        message_id = row.get("message_id", row.get(b"message_id"))
        delivered = row.get("times_delivered", row.get(b"times_delivered"))
        if not isinstance(delivered, int) or delivered < 1:
            raise RuntimeError("run_command_pending_invalid")
        counts[_text(message_id)] = delivered
    return counts


async def consume_run_command_batch(
    session_factory: async_sessionmaker[AsyncSession],
    redis: RunCommandRedis,
    *,
    consumer: str,
    batch_size: int = 20,
    reclaim_idle_ms: int = 30_000,
) -> dict[str, int]:
    if not 1 <= len(consumer) <= 120:
        raise ValueError("consumer_invalid")
    if not 1 <= batch_size <= _MAX_BATCH:
        raise ValueError("batch_size_invalid")
    if not 30_000 <= reclaim_idle_ms <= 3_600_000:
        raise ValueError("reclaim_idle_ms_invalid")

    claimed = _claimed_messages(
        await redis.xautoclaim(
            name=RUN_COMMANDS_STREAM,
            groupname=RUN_COMMANDS_GROUP,
            consumername=consumer,
            min_idle_time=reclaim_idle_ms,
            start_id="0-0",
            count=batch_size,
        )
    )
    fresh: list[tuple[str, Mapping[bytes, bytes]]] = []
    remaining = batch_size - len(claimed)
    if remaining > 0:
        fresh = _fresh_messages(
            await redis.xreadgroup(
                groupname=RUN_COMMANDS_GROUP,
                consumername=consumer,
                streams={RUN_COMMANDS_STREAM: ">"},
                count=remaining,
                block=1,
            )
        )
    combined = claimed + fresh
    counts = {
        "claimed": len(claimed),
        "fresh": len(fresh),
        "processed": 0,
        "duplicate": 0,
        "pending": 0,
        "deferred": 0,
        "rejected": 0,
    }
    if not combined:
        return counts

    pending = _delivery_counts(
        await redis.xpending_range(
            name=RUN_COMMANDS_STREAM,
            groupname=RUN_COMMANDS_GROUP,
            min="-",
            max="+",
            count=batch_size,
            consumername=consumer,
        )
    )
    seen: set[str] = set()
    for message_id, fields in combined:
        if message_id in seen:
            continue
        seen.add(message_id)
        result = await consume_run_command(
            session_factory,
            redis,
            StreamDelivery(
                stream=RUN_COMMANDS_STREAM,
                group=RUN_COMMANDS_GROUP,
                message_id=message_id,
                fields=fields,
                delivery_count=pending.get(message_id, 1),
            ),
        )
        counts[result] += 1
    return counts
