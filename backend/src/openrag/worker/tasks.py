...
def parse_task(task: Task, document_id: str) -> str:
    _run(task, lambda: ingest.run_parse(UUID(document_id)))
    return document_id
...
def chunk_task(task: Task, document_id: str) -> str:
    _run(task, lambda: ingest.run_chunk(UUID(document_id)))
    return document_id
...
def embed_upsert_task(task: Task, document_id: str) -> str:
    _run(task, lambda: ingest.run_embed_upsert(UUID(document_id)))
    return document_id
